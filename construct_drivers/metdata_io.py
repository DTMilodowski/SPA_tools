...
def load_all_metdata_for_randomforest(met_file, soil_file, ERA_file, TRMM_file, start_date, end_date, tstep = 0.5):
    ...
    # Second deal with RS data
    airT_RS = np.zeros(N_tsteps_out)*np.nan
    airT_p_RS = np.zeros(N_tsteps_out)*np.nan
    airT_n_RS = np.zeros(N_tsteps_out)*np.nan
    pptn_RS = np.zeros(N_tsteps_out)*np.nan
    pptn_d_RS = np.zeros(N_tsteps_out)*np.nan
    rh_RS = np.zeros(N_tsteps_out)*np.nan
    ...
    airT_RS[out_indices] = airT_mod[data_i1:data_i2]
    airT_n_RS[out_indices] = airT_n_mod[data_i1:data_i2]
    airT_p_RS[out_indices] = airT_p_mod[data_i1:data_i2]

    rh_n_RS[out_indices] = rh_n_mod[data_i1:data_i2]
    rh_p_RS[out_indices] = rh_p_mod[data_i1:data_i2]
    rh_RS[out_indices] = rh_mod[data_i1:data_i2]

    PAR_RS[out_indices] = PAR_mod[data_i1:data_i2]

    vpd_RS[out_indices] = vpd_mod[data_i1:data_i2]
    vpd_n_RS[out_indices] = vpd_n_mod[data_i1:data_i2]
    vpd_p_RS[out_indices] = vpd_p_mod[data_i1:data_i2]

    BP_RS[out_indices] = sp_mod[data_i1:data_i2]
    BP_n_RS[out_indices] = sp_n_mod[data_i1:data_i2]
    BP_p_RS[out_indices] = sp_p_mod[data_i1:data_i2]
    ...
    pptn_RS[out_indices] = TRMM_pptn[data_i1:data_i2]
    pptn_d_RS[out_indices] = TRMM_d_pptn[data_i1:data_i2]

    #--------------------------------------------------------------------------------------------
    print "\t6 - transferring data into output dictionaries"
    # now put all arrays into dictionaries so it is easy to access later  ########## CONTINUE HERE
    met_data_dict['date'] = output_time_series.copy()
    met_data_dict['airT'] = airT_station.copy()
    met_data_dict['pptn'] = pptn_station.copy()
    met_data_dict['rh'] = rh_station.copy()
